...
def seed_assistant_prompt_settings(apps, schema_editor):
    AssistantPromptSettings = apps.get_model("chatbot", "AssistantPromptSettings")
    if AssistantPromptSettings.objects.filter(pk=1).exists():
        return
    AssistantPromptSettings.objects.get_or_create(
        pk=1,
        defaults={
            "base_prompt_az": BASE_PROMPT_AZ,
            "base_prompt_en": BASE_PROMPT_EN,
        },
    )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("chatbot", "0002_assistantpromptnote_assistantpromptsettings"),
    ]

    operations = [
        migrations.RunPython(seed_assistant_prompt_settings, noop_reverse),
    ]
